...
%   ATTENTION!
%
%   THIS IS NOT the main segmentation function. This is a validation script that should be run when
...
%   
%   The main segmentation function is FFF_Line_Segmentation.m
%
%
%   Run this function by calling Validate_segmentation in the command window.
%   the validation function will run in a similar manner to that of the main function, but without any outputs.
disp (' ');
disp ('ATTENTION!');
...
disp (' ');
disp ('Run it in the same way as the main function, but without any outputs.');
disp ('/ATTENTION!');
disp (' ');
...
if testSegmentChoice == 1
    resultReposition_new = [contourRepositions; contourToRasterReposition];
    resultContour_new = indexContourAlt;
    resultRaster_new = index_raster;
    resultTransitionRaster_new = index_trans_raster;
    resultWholeWorkpiece_new = [resultContour_new(1,2) resultRaster_new(numofRasterLines,2)];
    resultExternalPattern_new = [resultContour_new(1,2) resultContour_new(numofContourLines,3)];
    resultInternalPattern_new = [resultRaster_new(1,3) resultRaster_new(numofRasterLines,2)];

    if testxticksChoice == 1
        resultReposition_new = convUni(sensorSignalNormalized, resultReposition_new, Fs);
        resultContour_new = convUni(sensorSignalNormalized, resultContour_new, Fs);
        resultRaster_new = convUni(sensorSignalNormalized, resultRaster_new, Fs);
        resultTransitionRaster_new = convUni(sensorSignalNormalized, resultTransitionRaster_new, Fs);
        resultWholeWorkpiece_new = convUni(sensorSignalNormalized, resultWholeWorkpiece_new, Fs);
        resultExternalPattern_new = convUni(sensorSignalNormalized, resultExternalPattern_new, Fs);
        resultInternalPattern_new = convUni(sensorSignalNormalized, resultInternalPattern_new, Fs);
    end

    resultReposition_new = obtainOutput(resultReposition_new(:,1),...
        resultReposition_new(:,2), resultReposition_new(:,3));
    resultContour_new = obtainOutput(resultContour_new(:,1),...
        resultContour_new(:,2), resultContour_new(:,3));
    resultRaster_new = obtainOutput(resultRaster_new(:,1),...
        resultRaster_new(:,3), resultRaster_new(:,2));
    resultTransitionRaster_new = obtainOutput(resultTransitionRaster_new(:,1),...
        resultTransitionRaster_new(:,3), resultTransitionRaster_new(:,2));

    StartPoint = resultWholeWorkpiece_new(1);
    EndPoint = resultWholeWorkpiece_new(2);

    if exist('OCTAVE_VERSION', 'builtin') == 0
        resultWholeWorkpiece_new = table(StartPoint,EndPoint);
    else
        resultWholeWorkpiece_new = struct();
        resultWholeWorkpiece_new.StartPoint = StartPoint;
        resultWholeWorkpiece_new.EndPoint = EndPoint;
    end


    StartPoint = resultExternalPattern_new(1);
    EndPoint = resultExternalPattern_new(2);

    if exist('OCTAVE_VERSION', 'builtin') == 0
        resultExternalPattern_new = table(StartPoint,EndPoint);
    else
        resultExternalPattern_new = struct();
        resultExternalPattern_new.StartPoint = StartPoint;
        resultExternalPattern_new.EndPoint = EndPoint;
    end

    StartPoint = resultInternalPattern_new(1);
    EndPoint = resultInternalPattern_new(2);

    if exist('OCTAVE_VERSION', 'builtin') == 0
        resultInternalPattern_new = table(StartPoint,EndPoint);
    else
        resultInternalPattern_new = struct();
        resultInternalPattern_new.StartPoint = StartPoint;
        resultInternalPattern_new.EndPoint = EndPoint;
    end
...
if testSegmentChoice == 1
    resultReposition_new = gen_signal_segments(sensorSignalNormalized,...
        contourRepositions);
    resultReposition_new(1,numofContourSides) = gen_signal_segments(sensorSignalNormalized,...
        contourToRasterReposition);
    resultContour_new = gen_signal_segments(sensorSignalNormalized, indexContour);
    resultRaster_new = gen_signal_segments(sensorSignalNormalized, index_raster);
    resultTransitionRaster_new = gen_signal_segments(sensorSignalNormalized, index_trans_raster);
    resultWholeWorkpiece_new = gen_signal_segments(sensorSignalNormalized, [resultContour_new(1,2) resultRaster_new(numofRasterLines,2)]);
    resultExternalPattern_new = gen_signal_segments(sensorSignalNormalized, [resultContour_new(1,2) resultContour_new(numofContourLines,3)]);
    resultInternalPattern_new = gen_signal_segments(sensorSignalNormalized, [resultRaster_new(1,3) resultRaster_new(numofRasterLines,2)]);
end
...
if size(resultReposition_new,1) == size(result_reposition,1) &&...
    size(resultContour_new,1) == size(result_contour,1) &&...
    size(resultRaster_new,1) == size(result_raster,1) &&...
    size(resultTransitionRaster_new,1) == size(result_transition_raster,1) &&...
    size(resultWholeWorkpiece_new,1) == size(resultWholeWorkpiece,1) &&...
    size(resultExternalPattern_new,1) == size(resultExternalPattern,1) &&...
    size(resultInternalPattern_new,1) == size(resultInternalPattern,1) &&...
    size(resultReposition_new,2) == size(result_reposition,2) &&...
    size(resultContour_new,2) == size(result_contour,2) &&...
    size(resultRaster_new,2) == size(result_raster,2) &&...
    size(resultTransitionRaster_new,2) == size(result_transition_raster,2) &&...
    size(resultWholeWorkpiece_new,2) == size(resultWholeWorkpiece,2) &&...
    size(resultExternalPattern_new,2) == size(resultExternalPattern,2) &&...
    size(resultInternalPattern_new,2) == size(resultInternalPattern,2)

    disp (' ');
    disp ('The segmentation results are with the expected dimensions.');
    disp ('This indicates that the segmentation behaved expectantly.');
    disp (' ');
else
    disp (' ');
    disp ('ATTENTION!');
    disp ('The segmentation results are not with the expected dimensions.');
    disp ('This indicates that the segmentation behaved unexpectantly.');
    disp ('');
    disp ('Follow the README execution instructions and run with the example data.');
    disp ('If the main function runs correctly, it appears that the problem was with the user selected data.');
    disp ('However, if the same problem occurs, open an issue in the repository.');
    disp ('/ATTENTION!')
    disp (' ');
end
...
function save_files(result_reposition, result_contour, result_raster,...
    result_transition_raster, resultWholeWorkpiece_new, resultExternalPattern_new,...
    resultInternalPattern_new, segmentation_choice, signal_identifier)
...
%   resultWholeWorkpiece_new is the segmentation results for the whole workpiece, resultExternalPattern_new is the segmentation results for the external pattern,
%   resultInternalPattern_new is the segmentation results for the internal pattern, segmentation_choice is the choice of segmentation logic,
...
save ([segmentation_choice,' segmentation results ',signal_identifier],...
    ...
    'resultWholeWorkpiece_new', 'resultExternalPattern_new',...
    'resultInternalPattern_new');
cd(oldFolder);
end
